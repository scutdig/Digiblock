...
import de.neemann.digiblock.draw.shapes.ObservableValueReader;
...
public class Wire implements Drawable, Movable, ObservableValueReader {
    ...
    public void drawTo(Graphic graphic, Style highLight) {
        ...
        if (value != null && p1.y == p2.y && wireLen > MIN_LABEL_WIRE_LEN && value.getBits() > 1) {
            de.neemann.digiblock.draw.graphics.Orientation ori;
            Vector pos = getRoundPos();
            if (showBits) {
                pos = pos.add(0, 3);
                ori = de.neemann.digiblock.draw.graphics.Orientation.RIGHTTOP;
            } else {
                pos = pos.add(0, -3);
                ori = de.neemann.digiblock.draw.graphics.Orientation.CENTERBOTTOM;
            }
        ...
        if (bits > 1 && p1.y == p2.y && wireLen >= minCrossLen && showBits) {
            ...
            graphic.drawText(numPos, Integer.toString(bits), de.neemann.digiblock.draw.graphics.Orientation.LEFTBOTTOM, Style.WIRE_BITS);
        }
