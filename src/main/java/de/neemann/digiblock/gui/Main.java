...
import de.neemann.digiblock.Verification.ApplicationIverilogTb;
...
public final class Main extends JFrame implements ClosingWindowListener.ConfirmSave, ErrorStopper, FileHistory.OpenInterface, DigitalRemoteInterface, StatusInterface, ChangedListener {
    ...
    private Main(MainBuilder builder) {
        ...
        try {
            ...
            try (FileOutputStream out = new FileOutputStream(marsTempFile);
                 InputStream in = ClassLoader.getSystemResourceAsStream("Mars4_5.jar")) {
                if (in != null)
                    IOUtils.copy(in, out);
            }
        } catch (IOException e) {
        ...
        try {
            ...
            try (FileOutputStream out = new FileOutputStream(armv8TempFile);
                 InputStream in = ClassLoader.getSystemResourceAsStream("ARMv8_Assembler.jar")) {
                if (in != null)
                    IOUtils.copy(in, out);
            }
        } catch (IOException e) {
